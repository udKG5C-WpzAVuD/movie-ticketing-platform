...
-- 移除错误的外键约束
--     添加此段sql在navicat运行即可
-- ALTER TABLE admin_movie_operation_logs DROP FOREIGN KEY fk_admin_movie_id;
--
-- -- 添加正确的外键（关联movies表）
-- ALTER TABLE admin_movie_operation_logs
--     ADD CONSTRAINT fk_log_movie_id FOREIGN KEY (movie_id) REFERENCES movies(id);
-- ----------------------------
